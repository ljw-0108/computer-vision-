...
img = cv.imread('C:\\Users\\user\\computer-vision-\\computer-vision-\\ch4\\soccer.jpg')  # 영상 읽기
...
cv.grabCut(img,mask,None,background,foreground,1,cv.GC_INIT_WITH_MASK) #전경과 배경을 구분하는 과정을 5번 반복해서 보정
# 5번 반복했을 때보다 1번 반복했을 때 객체의 경계와 일부 영역이 덜 정확하게 분리되었다.
# 반복 횟수가 줄어 전경과 배경을 충분히 보정하지 못했기 때문이다.
